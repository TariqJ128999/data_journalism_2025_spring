...
**Task** Load the five CSV files. Do they have the same column structure? **Yes they all have the same column structure the only difference is the year**
...
**Task** Load the Maryland 2020 county population data **I loaded in the Maryland Population data**
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **There is 24 rows, there should be 25 rows.
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **The output says "joining with 'by = join_by(COUNTY)**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Prince Georges County**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **The data type of the ZTCA5N is a number**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **The data type of ZCTA5N is now a character **
