...
**** I think that the code below is separating the data by the percentage of votes the candidate received. I say this because the code (election_day/votes)100 kind of tells me that it is doing some sort of math to find and average***
...
**The difference between task 5 and 6 is that, in task 6 it arranged it to have the candadites who had the highest percentage on election day are at the top**
...
**In my opinion its a more interesting way because you aren't allowed to see the district that the candidiate is from because alot of them say NA**
...
**str_to_lower(), what this function does is it changes all characters to lower case. Another on is str_to_title(), what this does is capatalize each word in a tittle and lowercases the rest**
...
group_by(amoutn_category) |>
summarise (
  contributions = n()
) |>
mutate( 
percent=(contributions/sum(contributions)*100)
  )
...
**The largest category is less than 100 dollars** 
...
library(lubridate)
maryland_expenses |>
  mutate(new_expanditure_date= mdy(expenditure_date))
...
**The new variable to hold the results is important because we know that the individuals are from prince Georges county but what this did was give us the office name and the candidate name, excluding all the unnecessary information that we didn't need  **
...
**Answer:The difference is that R wasn't see the date as a date rather as just a code.So now we can see the full date and year.**
...
I believe it is show the total amount of expenditures that was spent throughout the entire campaign.
